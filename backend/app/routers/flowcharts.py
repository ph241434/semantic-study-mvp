...
@router.get("/flowcharts", response_model=list[schemas.FlowchartSummary])
def list_flowcharts(knowledge_space_id: int | None = None, db: Session = Depends(get_db)):
    return service.list_flowcharts(db, knowledge_space_id)


@router.post("/flowcharts", response_model=schemas.FlowchartSummary, status_code=status.HTTP_201_CREATED)
def create_flowchart(flowchart_in: schemas.FlowchartCreate, db: Session = Depends(get_db)):
    try:
        flowchart = service.create_flowchart(db, flowchart_in)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    return service.flowchart_summary(db, flowchart)
...
@router.patch("/flowcharts/{flowchart_id}", response_model=schemas.FlowchartSummary)
def update_flowchart(flowchart_id: int, flowchart_in: schemas.FlowchartUpdate, db: Session = Depends(get_db)):
...
def create_flow_node(flowchart_id: int, node_in: schemas.FlowNodeCreate, db: Session = Depends(get_db)):
    flowchart = _flowchart_or_404(db, flowchart_id)
    try:
        return service.node_read(service.create_flow_node(db, flowchart, node_in))
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc


@router.patch("/flow-nodes/{node_id}", response_model=schemas.FlowNodeRead)
def update_flow_node(node_id: int, node_in: schemas.FlowNodeUpdate, db: Session = Depends(get_db)):
    node = _node_or_404(db, node_id)
    try:
        return service.node_read(service.update_flow_node(db, node, node_in))
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
@router.post("/flow-nodes/{node_id}/child-flowchart", response_model=schemas.FlowchartSummary)
def create_child_flowchart(node_id: int, db: Session = Depends(get_db)):
    child = service.create_child_flowchart(db, _node_or_404(db, node_id))
    return service.flowchart_summary(db, child)
...
def create_flow_edge(flowchart_id: int, edge_in: schemas.FlowEdgeCreate, db: Session = Depends(get_db)):
    flowchart = _flowchart_or_404(db, flowchart_id)
    try:
...
def update_flow_edge(edge_id: int, edge_in: schemas.FlowEdgeUpdate, db: Session = Depends(get_db)):
